...
void* say_hello2( void* args )
{
    ...
    cout << "time: "<< buf <<"."<<(int)(tv.tv_usec / 1000) <<" pid :"<< "[" << pid << "] in thread " <<  *( ( int* )args ) << endl;
	...
	if( tasks > BOUNDARY )
	{
	    cout << "time: "<< buf <<"."<<(int)(tv.tv_usec / 1000) <<" pid :"<< "[" << pid << "] take task: " << tasks << " in thread " << *( (int*)args ) << endl;
 	    --tasks; //modify
	}
	else if( !is_signaled )
	{
	    cout << "time: "<< buf <<"."<<(int)(tv.tv_usec / 1000) <<" pid :"<< "[" << pid << "] pthread_cond_signal in thread " << *( ( int* )args ) << endl;
...
int main()
{
    ...
    gettimeofday(&tv, NULL);                 //获取系统时间
    strftime(buf, sizeof(buf)-1, "%Y-%m-%d %H:%M:%S", localtime(&tv.tv_sec));
    cout << "time: "<< buf <<"."<<(int)(tv.tv_usec / 1000)<<endl;
    ...
    int index1 = 1;
    int ret = pthread_create( &tid1, &attr, say_hello2, ( void* )&index1 );
